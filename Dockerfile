...
ARG src_dir='/root/main'
ARG build_dir='/root/main/build'
ARG staging_dir='/root/main/staging'
...
COPY main.cpp "$src_dir/main.cpp"
...
RUN cmake --build "$build_dir" -t main -j "$(nproc)"  \
...
LABEL paulsengroup.main.image.build-base="$BUILD_BASE_IMAGE"
